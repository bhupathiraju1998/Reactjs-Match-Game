...
class App extends Component {
  ...
  clickMatchImage = id => {
    const {activeImgIndex, isGameOver} = this.state
    if (id === activeImgIndex.id) {
      this.setState(prevState => ({score: prevState.score + 1}))
      const index = Math.floor(Math.random() * imagesList.length)
      this.setState({activeImgIndex: imagesList[index]})
    } else {
      this.setState({isGameOver: !isGameOver})
  ...
  renderImages = () => {
    const {activeTabId, activeImgIndex} = this.state
    const filteredProjects = this.getFilteredProjects()
    const imgUrl = activeImgIndex.imageUrl
    return (
      <div className="inner-container">
        <div>
          <img src={imgUrl} alt="match" className="imgMatch" />
        </div>
        ...
        <ul className="project-list-container">
          {filteredProjects.map(imageDetails => (
            <ProjectItem
              key={imageDetails.id}
              imageDetails={imageDetails}
              clickMatchImage={this.clickMatchImage}
            />
          ))}
        </ul>
      </div>
  ...
  resetGame = () => {
    ...
    this.setState({isGameOver: !isGameOver, score: 0, timer: 60})
  ...
  render() {
    ...
    return (
      <div className="app-container">
        ...
        <div>{isGameOver ? this.renderImages() : this.renderResult()}</div>
      </div>
    )
  }
}
